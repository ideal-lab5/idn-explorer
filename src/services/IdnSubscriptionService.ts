...
export class IdnSubscriptionService implements ISubscriptionService {
  private subscriptionCache = new Map<
    string,
    { subscriptions: Subscription[]; timestamp: number }
  >();
  ...
  async killSubscription(signer: any, subscriptionId: string): Promise<void> {
    try {
      ...
      throw new Error(
        `Failed to kill subscription: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  async calculateSubscriptionFees(amount: number): Promise<number> {
    try {
      ...
      throw new Error(
        `Failed to calculate subscription fees: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  private async getSubscriptionsViaStorage(accountId: string): Promise<Subscription[]> {
    try {
      ...
      console.log(
        `Scanning ${Math.min(entries.length, maxSubscriptionsToScan)} subscriptions for account ${accountId}`
      );
      ...
      for (const [key, value] of entries) {
        ...
        if (scannedCount >= maxSubscriptionsToScan) {
          console.warn(
            `Reached scan limit of ${maxSubscriptionsToScan} subscriptions. Some subscriptions may not be loaded.`
          );
          break;
        }
        scannedCount++;
      ...
      console.log(
        `Found ${subscriptions.length} subscriptions for account ${accountId} after scanning ${scannedCount} entries`
      );
      return subscriptions;
    } catch (error) {
  ...
  private async getSubscriptionViaStorage(subscriptionId: string): Promise<Subscription | null> {
    try {
      ...
      console.log(
        `Scanning ${Math.min(entries.length, maxSubscriptionsToScan)} subscriptions for ID ${subscriptionId}`
      );
        ...
        try {
          if (value && !value.isEmpty) {
            ...
            if (subscription.id === subscriptionId) {
              console.log(
                `Found subscription ${subscriptionId} after scanning ${scannedCount} entries`
              );
              return subscription;
            }
          }
      ...
      console.log(
        `Subscription ${subscriptionId} not found after scanning ${scannedCount} entries`
      );
      return null;
    } catch (error) {
